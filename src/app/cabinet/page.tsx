...
export default function Profile(){
  ...
  // console.log(session)
  const {query, data: user, isLoading, errorMessage, setErrorMessage} = useQueryData()
  useEffect(()=>{
  ...
  return <>
    { userId ? 
      <>
        {errorMessage && <AlertError> Не удалось получить пользователя </AlertError>}
...
function getMenuItems(id: string | number) {
  return menu.map(i => ({...i, href: i.href.replace('[userId]', String(id))}))
} 
...
function UserProfile({user}: {user: User}){
  ...
  function createReview() {
    ...
    APIReview.create(newReview)
      .then((res) => { router.push(`/users/${id}/reviews`) }) 
  }
